...
[`knitr::spin()`](https://yihui.org/knitr/demo/stitch/), you might
wonder how `quartify` differs:
...
3.  **Advanced Outputs**: Generate websites, books, presentations, and
    scientific articles with the same workflow
4.  **Better Theming**: Access to modern, responsive HTML themes with
    consistent styling
...
The `Description` field can span multiple lines. To continue the
description on a new line:

- Start the next line with `#` followed by **at least one space** and
...
**⚠️ Important for text conversion:**

For a comment to be correctly converted to Quarto text, the line must
start with `#` **without any leading space**. If you have indentation
spaces before the `#`, the comment will be ignored and remain in the
code chunk.
...
If your R script contains roxygen2 documentation blocks (lines starting
with `#'`), **quartify** automatically converts them into elegant
callout-note sections with the function name:
...
The difference:  
- **Comments at the start of a line** (standalone) → become text OUTSIDE
code blocks  
- **Comments within code** (indented or part of a pipeline) → stay
INSIDE code blocks
...
Now that you’ve learned the basics of `quartify`, explore more advanced
features:
...
- **Customization Options**: Themes, rendering options, output
  configuration
- **Batch Conversion**: Convert entire directories with
  [`rtoqmd_dir()`](https://ddotta.github.io/quartify/reference/rtoqmd_dir.md)
- **Quarto Books**: Automatically create navigable book projects
- **Code Quality Integration**: styler and lintr for code formatting and
  quality checks
